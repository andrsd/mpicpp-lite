...
MatchedMessage::count() const
{
    assert(this->message_ != MPI_MESSAGE_NULL);
    int n;
    MPI_CHECK_SELF(MPI_Get_count(&this->status_.native(), mpi_datatype<T>(), &n));
    return n;
}
...
MatchedMessage::recv(T * values, int n)
{
    ...
    Status status;
    MPI_CHECK_SELF(MPI_Mrecv(values, n, mpi_datatype<T>(), &this->message_, &status.native()));
    return status;
}
...
MatchedMessage::recv()
{
    ...
    Status status;
    MPI_CHECK_SELF(MPI_Mrecv(MPI_BOTTOM, 0, MPI_PACKED, &this->message_, &status.native()));
    return status;
}
