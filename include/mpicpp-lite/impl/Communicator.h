...
public:
    ...
    /// @param op Reduce operation
    template <typename T, typename Op>
    void exscan(const T * in_values, int n, T * out_value, Op op) const;
    ...
    /// @param op Reduce operation
    template <typename T, typename Op>
    void exscan(const std::vector<T> & in_values, std::vector<T> & out_values, Op op) const;
...
public:
    ...
    create_key(void * extra_state = nullptr)
    {
        ...
        MPI_CHECK(MPI_Comm_create_keyval(MPI_COMM_NULL_COPY_FN,
                                         ...
                                         &keyval,
                                         extra_state));
        Environment::comm_key_vals_.push_back(keyval);
        return Key { keyval };
    }
};
...
Communicator::create(const Group & group, Tag tag) const
{
    ...
    MPI_CHECK_SELF(MPI_Comm_create_group(this->comm_, group.group_, tag.value(), &new_comm.comm_));
    return new_comm;
}
...
template <typename T>
inline void
Communicator::send(int dest, Tag tag, const T * values, int n) const
{
    MPI_CHECK_SELF(
        MPI_Send(const_cast<T *>(values), n, mpi_datatype<T>(), dest, tag.value(), this->comm_));
}
...
template <>
inline void
Communicator::send(int dest, Tag tag, const std::string & value) const
{
    if (size() < 2)
        return;
...
template <typename T>
inline Status
Communicator::recv(int source, Tag tag, T * values, int n) const
{
    Status status;
...
template <>
inline Status
Communicator::recv(int source, Tag tag, std::string & value) const
{
    std::vector<char> str;
...
template <typename T>
inline Request
Communicator::isend(int dest, Tag tag, const T & value) const
{
...
template <typename T>
inline Request
Communicator::isend(int dest, Tag tag, const T * values, int n) const
{
...
template <typename T>
inline Request
Communicator::irecv(int source, Tag tag, T * values, int n) const
{
    assert(values != nullptr);
    Request request;
...
inline bool
Communicator::iprobe(int source, Tag tag, Status & status) const
{
    int flag;
    MPI_CHECK_SELF(MPI_Iprobe(source, tag.value(), this->comm_, &flag, &status.native()));
    return flag != 0;
...
Communicator::reduce(int n, T * values, Op, int root) const
{
    ...
    if (rank() == root)
        MPI_CHECK_SELF(
            MPI_Reduce(MPI_IN_PLACE, values, n, mpi_datatype<T>(), mpi_op, root, this->comm_));
    else
        MPI_CHECK_SELF(MPI_Reduce(values, NULL, n, mpi_datatype<T>(), mpi_op, root, this->comm_));
}
...
template <typename T, typename Op>
inline void
Communicator::exscan(const std::vector<T> & in_values, std::vector<T> & out_values, Op op) const
{
    assert(in_values.size() == out_values.size());
    exscan(in_values.data(), in_values.size(), out_values.data(), op);
}
...
Communicator::attr(Key key) const
{
    T * val = nullptr;
    int flag = 0;
    MPI_CHECK_SELF(MPI_Comm_get_attr(this->comm_, key.value(), &val, &flag));
    if (flag)
        return *val;
    else
        return std::nullopt;
